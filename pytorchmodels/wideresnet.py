...
__all__ = ['CIFARWRN', 'SeparatedCIFARWRN', 'wrn28_10_cifar10_two', 'wrn16_10_cifar10', 'wrn16_10_cifar100', 'wrn16_10_svhn', 'wrn28_10_cifar10',
...
class SeparatedCIFARWRN(nn.Module):
    """
    ...
    def __init__(self,
                 ...
                 in_size=(32, 32),
                 num_classes=10):
        super(SeparatedCIFARWRN, self).__init__()
        self.in_size = in_size
        self.num_classes = num_classes

        self.feature_extractor = SeparatedCIFARWRN_F(channels[:-1],
                 init_block_channels,
                 in_channels=3,
                 in_size=(32, 32),
                 num_classes=10)
        self.discriminator = SeparatedCIFARWRN_D([channels[-1]],
                 channels[:-1][-1][0],
                 in_channels=3,
                 in_size=(32, 32),
                 num_classes=10)
...
class SeparatedCIFARWRN_F(nn.Module):
    def __init__(self,
                 channels,
                 init_block_channels,
                 in_channels=3,
                 in_size=(32, 32),
                 num_classes=10):
        super(SeparatedCIFARWRN_F, self).__init__()
        self.in_size = in_size
        self.num_classes = num_classes

        self.add_module("init_block", conv3x3(
            in_channels=in_channels,
            out_channels=init_block_channels))
        in_channels = init_block_channels
        for i, channels_per_stage in enumerate(channels):
...
class SeparatedCIFARWRN_D(nn.Module):
    def __init__(self,
                 channels,
                 init_block_channels,
                 in_channels=3,
                 in_size=(32, 32),
                 num_classes=10):
        super(SeparatedCIFARWRN_D, self).__init__()
        self.in_size = in_size
        self.num_classes = num_classes
        ...
        self.add_module("post_activ", PreResActivation(in_channels=in_channels))
...
def wrn28_10_cifar10_two(num_classes=10, **kwargs):
    """
    ...
    root : str, default '~/.torch/models'
        Location for keeping the model parameters.
    """
    return get_wrn_cifar(num_classes=num_classes, blocks=28, width_factor=10, separate=True, model_name="wrn28_10_cifar10", **kwargs)
