...
__all__ = ['SeparatedCIFARResNet', 'CIFARResNet', 'resnet20_cifar10_two', 'resnet56_cifar10_two', 
...
class CIFARResNet(nn.Module):
    ...
    def forward(self, x, out_feature=False, intermediate=False):
        if intermediate:
            self.x1 = self.features.init_block(x)
            self.x2 = self.features.stage1(self.x1)
            self.x3 = self.features.stage2(self.x2)
            self.x4 = self.features.stage3(self.x3)
            self.x5 = self.features.final_pool(self.x4)
            feature = self.x5.view(self.x5.size(0), -1)
            x = self.output(feature)
            return x #, {'init_block':self.x1, 'stage1':x2, 'stage2':x3, 'stage3':x4} #for pca
        else:
            x = self.features(x)
            feature = x.view(x.size(0), -1)
            x = self.output(feature)
...
class SeparatedCIFARResNet(nn.Module):
    """
    ResNet model for CIFAR from 'Deep Residual Learning for Image Recognition,' https://arxiv.org/abs/1512.03385.
    ...
    def __init__(self,
                 ...
                 in_size=(32, 32),
                 num_classes=10):
        super(SeparatedCIFARResNet, self).__init__()
        self.in_size = in_size
        self.num_classes = num_classes

        self.feature_extractor = SeparatedCIFARResNet_F(channels[:-1],
                 init_block_channels,
                 bottleneck,
                 in_channels=3,
                 in_size=(32, 32),
                 num_classes=10)
        self.discriminator = SeparatedCIFARResNet_D([channels[-1]],
                 channels[:-1][-1][0],
                 bottleneck,
                 in_channels=3,
                 in_size=(32, 32),
...
class SeparatedCIFARResNet_F(nn.Module):
    def __init__(self,
                 channels,
                 init_block_channels,
                 bottleneck,
                 ...
                 in_size=(32, 32),
                 num_classes=10):
        super(SeparatedCIFARResNet_F, self).__init__()
        self.in_size = in_size
        self.num_classes = num_classes


        self.add_module("init_block", conv3x3_block(
            in_channels=in_channels,
            out_channels=init_block_channels))
        in_channels = init_block_channels
        for i, channels_per_stage in enumerate(channels):
...
class SeparatedCIFARResNet_D(nn.Module):
    def __init__(self,
                 ...
                 in_size=(32, 32),
                 num_classes=10):
        super(SeparatedCIFARResNet_D, self).__init__()
        self.in_size = in_size
        self.num_classes = num_classes
        ...
        for i, channels_per_stage in enumerate(channels):
            ...
            for j, out_channels in enumerate(channels_per_stage):
                ...
                stage.add_module("unit{}".format(j + 1), ResUnit(
                    ...
                    conv1_stride=False))
                in_channels = out_channels
            self.add_module("stage{}".format(i + 3), stage)
...
def resnet20_cifar10_two(num_classes=10, **kwargs):
    """
    ...
    root : str, default '~/.torch/models'
        Location for keeping the model parameters.
    """
    return get_resnet_cifar(num_classes=num_classes, blocks=20, bottleneck=False, model_name="resnet20_cifar10", separate=True,
                            **kwargs)
def resnet56_cifar10_two(num_classes=10, **kwargs):
    """
    ...
    root : str, default '~/.torch/models'
        Location for keeping the model parameters.
    """
    return get_resnet_cifar(num_classes=num_classes, blocks=56, bottleneck=False, model_name="resnet56_cifar10", separate=True,
                            **kwargs)
